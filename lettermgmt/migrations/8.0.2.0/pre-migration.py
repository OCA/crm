...
def migrate(cr, version):
    ...
    cr.execute(
        """UPDATE ir_model_fields SET relation = 'letter.category'
                  WHERE relation = 'letter.class'"""
    )
    cr.execute(
        """UPDATE ir_model_fields SET model = 'letter.category'
                  WHERE model = 'letter.class'"""
    )
    cr.execute(
        """UPDATE ir_model_data SET model = 'letter.category'
                  WHERE model = 'letter.class'"""
    )
    cr.execute(
        """UPDATE ir_attachment SET res_model = 'letter.category'
