...
class ResLetter(models.Model):
    ...
    date = fields.Date(
        string="Letter Date", help="The letter's date.", default=fields.Date.today
    )
    snd_date = fields.Date(string="Sent Date", help="The date the letter was sent.")
    rec_date = fields.Date(
        string="Received Date", help="The date the letter was received."
    )
    ...
    note = fields.Text(
        string="Delivery Notes", help="Indications for the delivery officer."
    )

    channel_id = fields.Many2one(
        "letter.channel", string="Channel", help="Sent / Receive Source"
    )
    ...
    folder_id = fields.Many2one(
        "letter.folder", string="Folder", help="Folder which contains letter."
    )

    type_id = fields.Many2one(
        "letter.type", string="Type", help="Type of Letter, Depending upon size."
    )
    ...
    track_ref = fields.Char(
        string="Tracking Reference", help="Reference Number used for Tracking."
    )
    orig_ref = fields.Char(
        string="Original Reference", help="Reference Number at Origin."
    )
    expeditor_ref = fields.Char(
        string="Expeditor Reference", help="Reference Number used by Expeditor."
    )

    parent_id = fields.Many2one(
        "res.letter", string="Parent", groups="lettermgmt.group_letter_thread"
    )
