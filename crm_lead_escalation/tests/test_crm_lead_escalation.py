...
class TestCrmLeadEscalation(TransactionCase):
    ...
    def test_mail_never_reaches_the_customer(self):
        """The escalation warns the recipient, never the customer of the lead."""
        customer = self.env["res.partner"].create(
            {"name": "Test Customer", "email": "customer@test.example.com"}
        )
        with_partner = self._create_lead(partner_id=customer.id)
        without_partner = self._create_lead(email_from="prospect@test.example.com")
        for lead in with_partner + without_partner:
            self._set_stale(lead)
        self._run_cron()
        for lead in with_partner + without_partner:
            mails = self._escalation_mails(lead)
            self.assertEqual(mails.recipient_ids, self.leader.partner_id)
